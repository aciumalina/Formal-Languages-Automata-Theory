...
# State Antonia
import copy
with open("tm_config_file2.txt") as fin:
    ok = 0
    ok1 = 0
    ok2 = 0
    ok3 = 0
...
#Pentru a trata cazul TM cu doua benzi si un singur cap, transformam TM-ul din doua benzi
#intr-un TM cu o singura banda in care vom face delimitarea dintre cele doua benzi folosind
#semnul '$'.
#Pentru a exemplifica o TM cu doua benzi si un cap, aplicam functia delta incepand cu prima banda.
#In momentul in care am ajuns la o stare de accept sau reject, continuam cu functia delta care ne va duce pe cea de-a doua banda, folosita pentru a face verificarea
#corectitudinii output-ului primei benzi.
#Functia pentru a trece de la prima banda la a doua este urmatoarea:
#delta(qa, $) = q1, $, R
#delta(qr, $) = q1, $, R
#Liniile anteriore indica faptul ca daca am ajuns in starea de accept(qa) sau starea de reject (qr), daca intalnim simbolul de delimitare dintre cele doua benzi,
#vom sti sa ne deplasam la dreapta (adica inceputul celei de-a doua benzi) cu starea de start (q1 in acest caz).
#Pentru a valida sau respinge input-ul, trebuie ca ambele benzi sa ajunga in starea de accept, respectiv reject.




banda1 = []                             #pe prima banda, denumita banda1, vom scrie input-ul caracter cu caracter
for i in cuvant:
    banda1.append(i)
banda2 = copy.deepcopy(banda1)          #cea de-a doua banda, denumita banda2, devine copia primei benzi
banda1.insert(0, stare_start)           #de asemenea, pentru a marca inceputul procesarii input-ului, vom altera lista "banda1" prin adaugarea starii de start
                                        #la inceput de input, pentru a arata starea curenta si pozitia la care ne aflam la un anumit moment.
...
banda.append('$')
banda.append(banda2)
#prin cele 4 linii de cod anterioare, am "concatenat" cele doua benzi intr-o singura banda, delimitandu-le prin semnul '$'
#banda = [banda1, '$', banda2]

stare1 = banda1[0]              #stare1 reprezinta starea de start a primei benzi
lista_prev1 = banda1
lista_prev2 = banda2

#In continuare am definit doua functii identice, functie1 si functie2 care actioneaza pe prima banda, respectiv a doua banda.

def functie1(delta, banda1, banda, lista_prev1):
    ...
    if (stare1 == stare_accept):            #cand ajungem cu starea1 in stare accept, inseamna ca inputul a fost acceptat pe prima banda
        ...
        lista_prev = lista_curenta          #in lista_prev vom retine lista_curenta
        banda[0] = banda1                   #prin aceasta linie vom reflecta la nivelul intregii benzi ce modificari s-au facut la banda 1
        for el in banda:
            if type(el) == list:
                for i in el:
                    print(i, end=" ")
        ...
        functie1(delta, banda1, banda, lista_prev)          #reapelare functie



#functia 2 este aceeasi ca functia 1, aplica functia de tranzitie pe a doua banda
...
functie1(delta, banda1, banda, lista_prev1)
#La terminarea functiei 1, inseamna ca am ajuns in stare de accept sau reject si am terminat aplicarea lui delta pe prima banda.
...
banda2.insert(0, delta[tuplu][0])       #vom insera in banda doi starea in care ajungem prin aplicarea functiei de tranzitie
stare2 = banda2[0]                      #stare2 devine starea de start pentru banda 2
