...
s = input('s=')
o = pct_st               #de unde incepem
index_curent_sir_input = 0
while index_curent_sir_input < len(s):
    ok = 0
    for t in transitions[o]:
        # t[0] - litera
        # l-pozitia curenta din sir
        if t[0] == s[index_curent_sir_input]:
            o = t[1]
            ok = 1

    if ok == 1:
        index_curent_sir_input += 1
    else:
        break

# verificam daca ultima stare in care am ajuns este una finala (in lista z avem toate starile finale) si o ultima stare obtinuta
if (o in z) and ok == 1:
